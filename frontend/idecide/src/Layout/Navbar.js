			...
			{!isLoading && surveys ? (
				<div className={classes.root}>
						...
						<Toolbar>
							...
							<Grid container direction="row" justify="flex-end" alignItems="center" spacing={2}>
								...
								localStorage.getItem('userType') !== 'anonymous' ? (
									<Grid container direction="row" justify="flex-end" alignItems="center" spacing={2}>
										<Grid item>
											<Button onClick={handleLogOut} className={clsx(classes.button)}>
												<ExitToAppIcon />
					...
					>
						...
						<Divider />
						<List>
							<NavLink to={'/dashboard/surveys'} onMouseDown={() => setShowSurvey(!showSurvey)}>
								<ListItem button>
									<ListItemIcon>
										<BallotIcon />{' '}
									</ListItemIcon>
							...
							<Collapse in={showSurvey} timeout="auto" unmountOnExit>
								{surveys &&
									surveys.map((survey, index) => (
										<NavLink to={'/dashboard/surveyId=' + survey.surveyId} key={survey.surveyId}>
