...
export default class SurveyHome extends Component {
	constructor(props) {
		...
		this.surveyOrder = [ 'My Situation', 'My Safety', 'My Priorities', 'Action Plan' ]; // order that surveys need to be completed in
	...
	async componentDidMount() {
		...
		if (userId === undefined) {
			console.log('no user ID passed in as prop to surveyHome... using Id =92138918723');
			userId = 92138918723;
		}
	...
	completeHandler = (surveyResults) => {
		...
		this.setState((prevState) => {
			...
			var newSurveyCompletions = [ ...prevState.surveyCompletions, surveyResults ];
			console.log('new survey completions is', newSurveyCompletions);
			localStorage.setItem('prevCompletions', JSON.stringify(newSurveyCompletions));
		...
		if (currentState === 'survey') {
			...
			renderElements.push(
				<div style={{ padding: '3%' }}>
					...
					<br />
          <Typography color="textPrimary"  gutterBottom variant="h5">Completing the modules below will help us better understand your situation</Typography>
          <Typography color="textPrimary"  gutterBottom variant="h5">So we can generate a personalised action plan for you.</Typography>
					<br />
						...
						<div key={survey.surveyId} className="surveyIcon">
							<SurveySelectionButton
								notAvailable={
									...
									survey.surveyTitle !== this.surveyOrder[this.state.nextSurvey]
								}
			...
			renderElements.push(<SurveyResultsPage returnHome={this.returnHomeCallback} />);
		} else if (currentState === 'actionPlan') {
			renderElements.push(<ActionPlans />);
		} else {
			renderElements.push(<LoadingSpinner />);
		}
		if (this.surveyOrder[this.state.nextSurvey] !== 'Action Plan' && this.state.currentState === 'menu') {
			renderElements.push(
				<Card style={{ position: 'fixed', bottom: 0, width: '100%' }}>
					<Card.Body>Please complete the required sections before we generate an action plan</Card.Body>
				</Card>
			);
